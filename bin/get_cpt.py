...
basedir = os.path.join('/Users/ajw1g19/Library/CloudStorage/OneDrive-UniversityofSouthampton/INSPIRE DTP/Projects/Paper_1_UKESM_MH_NAmerica/UKESM1.1_MH_NorthAmerica-v1.0/bin','cpt') # base path from where cpt files are fetched

def get_cmap(cpt_path, name=None, method='cdict', N=None, reverse=False):
    ...
    if 'http://' in cpt_path or 'https://' in cpt_path:
        with urlopen(cpt_path) as f:
            return gmtColormap_openfile(f, name=name, method=method, N=N, ret_cmap_type='LinearSegmented', reverse=reverse)
...
def gmtColormap_openfile(cptf, name=None, method='cdict', N=None, ret_cmap_type='LinearSegmented', reverse=False):
